  ...
  describe '.call' do
    context 'when query is nil' do
      it 'returns all clients' do
        result = described_class.call(sample_clients, nil)
    ...
    context 'when query is provided' do
      ...
      it 'handles clients with empty full_name' do
        result = described_class.call(sample_clients, 'empty')
        expect(result).to be_empty
      end
      ...
      it 'handles empty query string' do
        result = described_class.call(sample_clients, '')
    ...
    context 'when searching for existing clients' do
      it 'finds clients with exact name matches' do
        result = described_class.call(sample_clients, 'John Doe')
        expect(result).to contain_exactly(sample_clients[0])
      end
      ...
      it 'returns empty array for non-matching queries' do
        result = described_class.call(sample_clients, 'nonexistent')
        expect(result).to be_empty
      end
    end
    ...
    context 'when dealing with edge cases' do
      ...
      it 'handles very long names' do
        ...
        result = described_class.call([long_name_client], 'long')
        expect(result).to contain_exactly(long_name_client)
      end
    end

    context 'with special character names' do
      ...
      it 'finds client with hyphen in name' do
        result = described_class.call(special_clients, 'john')
        expect(result).to contain_exactly(special_clients[0])
      end

      it 'finds client with underscore in name' do
        result = described_class.call(special_clients, 'jane')
        expect(result).to contain_exactly(special_clients[1])
      end
    end
  end
end
